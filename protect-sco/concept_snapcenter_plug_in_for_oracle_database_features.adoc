...
+
** Para deshabilitar esto, debe configurarse el valor de ENABLE_LUNPATH_MONITORING como false.
** Es posible especificar la frecuencia en la cual se corrigen automáticamente las rutas de LUN mediante la asignación del valor (en horas) como LUNPATH_MONITORING_INTERVAL. Para obtener más información sobre SLM, consulte http://docs.netapp.com/ontap-9/topic/com.netapp.doc.dot-cm-sanag/home.html["Guía de administración de SAN de ONTAP 9"^].
